...
describe("originAllowlist", () => {
  it("allows matching string origins", async () => {
    const app = createApp(
      originAllowlist({ origins: ["https://example.com", "https://app.test"] }),
    );
  ...
  it("allows matching regex origins", async () => {
    const app = createApp(
      originAllowlist({ origins: [/https:\/\/.*\.example\.com$/] }),
    );
...
describe("payloadLimits", () => {
  it("allows normal payloads", async () => {
    const app = createApp(
      payloadLimits({ maxPayloadBytes: 10_000, maxScreenshotBytes: 5_000 }),
    );
  ...
  it("allows GET requests without checking", async () => {
    const app = createApp(
      payloadLimits({ maxPayloadBytes: 1, maxScreenshotBytes: 1 }),
    );
  ...
  it("rejects oversized screenshots", async () => {
    ...
    const app = createApp(
      payloadLimits({ maxPayloadBytes: 10_000, maxScreenshotBytes: 100 }),
    );
  ...
  it("rejects oversized event payloads", async () => {
    const app = createApp(
      payloadLimits({ maxPayloadBytes: 50, maxScreenshotBytes: 5_000_000 }),
    );
