...
describe('initSnapfeed click handling', () => {
  beforeEach(() => {
    ...
    mocks.sanitizeDetail.mockImplementation((detail: Record<string, unknown>) => detail)
  ...
  it('records clicks and rage-clicks through the same tracked click pipeline', () => {
    ...
    expect(eventTypes.filter((eventType) => eventType === 'click')).toHaveLength(3)
    expect(eventTypes.filter((eventType) => eventType === 'rage_click')).toHaveLength(1)
