...
function gatherBaseContext(el: Element): Record<string, unknown> {
  ...
  while (current && current !== document.body) {
    ...
    if (current.tagName === "IMG" && !ctx.img_src) {
      ctx.img_src = (current as HTMLImageElement).src.replace(
...
function buildBreadcrumb(baseContext: Record<string, unknown>): string {
  ...
  if (baseContext.img_src) {
    const fileName = (baseContext.img_src as string)
      .split("/")
      .pop()
      ?.split("?")[0];
...
async function captureScreenshot(
  ...
  try {
    ...
    const dataUrl = finalCanvas.toDataURL(
      "image/jpeg",
      config.feedback.screenshotQuality,
    );
  ...
  return {
    ...
    async submit() {
      ...
      push(
        ...
        text,
        detail,
        state.includeScreenshot ? screenshot : null,
      );
